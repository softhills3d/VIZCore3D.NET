    ...
    public partial class FrmMain : Form
    {
        ...
        private void timerProcess_Tick(object sender, EventArgs e)
        {
            ...
            foreach (string path in Arguments)
            {
                ...
                {
                    // dir.
                    string [] rev = System.IO.Directory.GetFiles(path, "*.rev", System.IO.SearchOption.TopDirectoryOnly);
                    string [] rvm = System.IO.Directory.GetFiles(path, "*.rvm", System.IO.SearchOption.TopDirectoryOnly);

                    List<string> items = new List<string>();
                    if(rev != null && rev.Length != 0)
                        items.AddRange(rev.ToList());

                    if (rvm != null && rvm.Length != 0)
                        items.AddRange(rvm.ToList());
        ...
        private void ConvertToVIZ(string path)
        {
            string name = System.IO.Path.GetFileName(path).ToUpper();
