    ...
    public partial class FrmMain : Form
    {
        ...
        private void Model_OnModelOpenedEvent(object sender, EventArgs e)
        {
            if(ckDraftView.Checked)
            {
                ...
                vizcore3d.Object3D.Transform.Rotate(new List<int>() { 0 }, 50, -40, -30, true, false);

                vizcore3d.View.SetRenderMode(Data.RenderModes.HIDDEN_LINE_REMOVAL);
            ...
            List<VIZCore3D.NET.Data.Node> nodes = vizcore3d.Object3D.FromFilter(Data.Object3dFilter.FILE);

            vizcore3d.View.MouseBasedObjectMove.Clear();

            for (int i = 0; i < nodes.Count; i++)
            {
                vizcore3d.View.MouseBasedObjectMove.Add(i, nodes[i].Index);
            }
        }

        private void MouseBasedObjectMove_OnMouseDownEvent(object sender, Event.EventManager.MouseBaseObjectMoveEventArgs e)
        {
        }
        ...
        private void btnDisassemble_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < nodes.Count; i++)
            {
                vizcore3d.Object3D.Disassembly.AddGroup(i, nodes[i].Index, true);
            }

            vizcore3d.Object3D.Disassembly.DisassembleBySphereCenterDistanceRate(true, 1.5f);
        }
