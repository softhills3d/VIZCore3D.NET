...
namespace VIZCore3D.NET.Panorama
{
    public partial class FrmMain : Form
    {
        ...
        private void btnLoadSample_Click(object sender, EventArgs e)
        {
            ...
            dlg.Filter = "Image File (*.*)|*.jpg;*.png";

            if (dlg.ShowDialog() != DialogResult.OK) return;

            LoadSample(dlg.FileNames);
        }

        private void LoadSample(string[] files)
        {
            lvSample.BeginUpdate();

            lvSample.Items.Clear();

            foreach (string item in files)
            {
                ListViewItem lvi = new ListViewItem(new string[] { System.IO.Path.GetFileName(item) });
                lvi.Tag = item;

                lvSample.Items.Add(lvi);
            }

            lvSample.EndUpdate();
        }

        private void lvSample_DoubleClick(object sender, EventArgs e)
        {
            if (lvSample.SelectedItems.Count == 0) return;

            ListViewItem lvi = lvSample.SelectedItems[0];
            if (lvi.Tag == null) return;

            string path = (string)lvi.Tag;

            vizcore3d.Panorama.Open(path);
        }
        ...
        private void btnAddCustomObject_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < 360; i += 10)
            {
                ...
                if (i % 40 == 0)
                {
                    ...
                    vizcore3d.Panorama.CustomObject.AddText(text, Data.FontSizeKind.SIZE20, color, fAngleX, fAngleY);
                }
                else
                {
            ...
            {
                List<Data.Vertex3D> position2 = new List<Data.Vertex3D>();
                for (int i = -90; i <= 90; i++)
                {
                    float fAngleX = 0;
                    float fAngleY = i;

                    position2.Add(new Data.Vertex3D(fAngleX, fAngleY, 0.0f));
                }

                Color color = Color.FromArgb(255, 255, 128, 0);
                vizcore3d.Panorama.CustomObject.AddPolyLine(position2, color, 2.0f, true);
            }
        ...
        {
            vizcore3d.Panorama.CustomObject.Clear();
        }
    }
}
