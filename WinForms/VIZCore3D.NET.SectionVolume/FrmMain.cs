...
namespace VIZCore3D.NET.SectionVolume
{
    public partial class FrmMain : Form
    {
        ...
        {
            if (ckAuto.Checked == false) return;
            ...
            switch (e.EventType)
            {
                ...
                case Manager.SectionManager.EventTypes.SELECT:
                    break;
                case Manager.SectionManager.EventTypes.TRANSFORM: // 진행중
                    //UpdateGeometryProperty();
                    break;
                case Manager.SectionManager.EventTypes.RESIZED: // 완료
                    UpdateGeometryProperty();
                    break;
                default:
                    break;
        ...
        private void btnSectionPlane_Click(object sender, EventArgs e)
        {
            if (vizcore3d.Model.IsOpen() == false) return;

            vizcore3d.Section.Clear();
            vizcore3d.Section.Add(false, VIZCore3D.NET.Data.Axis.X);
        }

        private void btnSectionBox_Click(object sender, EventArgs e)
        {
            if (vizcore3d.Model.IsOpen() == false) return;
        ...
        private void UpdateGeometryProperty()
        {
            this.Invoke(new EventHandler(delegate
            {
                ...
                {
                    lvErrorParts.BeginUpdate();
                    lvErrorParts.Items.Clear();
                    foreach (int index in prop.ErrorIndex)
                    {
