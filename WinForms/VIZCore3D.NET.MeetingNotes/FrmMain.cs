...
namespace VIZCore3D.NET.MeetingNotes
{
    public partial class FrmMain : Form
    {
        ...
        public VIZCore3D.NET.Data.NoteStyle noteStyle;
        ...
        {
            bool result = vizcore3d.Model.OpenFileDialog();

            if (result == true) InitNoteStyle();
        }

        private void InitNoteStyle()
        {
            noteStyle = vizcore3d.Review.Note.GetStyle();
            noteStyle.TextVisible = false;
            noteStyle.SymbolFontColor = Color.Black;
            noteStyle.SymbolBackgroundColor = Color.Yellow;
            noteStyle.SymbolSize = 18;
            noteStyle.SymbolFontSize = VIZCore3D.NET.Data.FontSizeKind.SIZE20;
            noteStyle.SymbolFontBold = true;

            vizcore3d.Review.Note.SetStyle(noteStyle);
        }
        ...
        private void btnStopNote_Click(object sender, EventArgs e)
        {
            EnableMode = false;
        }
        ...
        {
            if (EnableMode == false) return;
            if (e.Button != MouseButtons.Left) return;

            VIZCore3D.NET.Data.Vertex3D world = vizcore3d.View.ScreenToWorld(e.X, e.Y, 0, true);
            ...
            VIZCore3D.NET.Data.PickedSurfaceData vector =
                        vizcore3d.View.GetPickedSurfaceNormalVector(e.Location.X, e.Location.Y);

            VIZCore3D.NET.Data.Vertex3D position = vector.Position;

            int noteId = vizcore3d.Review.Note.AddNoteSurface("Note...", position, position);

            if (noteStyle == null) InitNoteStyle();

            VIZCore3D.NET.Utility.ColorPaletteHelper palette = new Utility.ColorPaletteHelper();
            palette.PaletteName = "Default";

            noteStyle.UseSymbol = true;
            noteStyle.UseTextBox = false;
            noteStyle.SymbolBackgroundColor = palette.GetPaletteColor(NoteNo % palette.PaletteCount).Color1;
            noteStyle.SymbolText = NoteNo.ToString();
            vizcore3d.Review.Note.SetStyle(noteId, noteStyle);

            ListViewItem lvi = new ListViewItem(new string[] { NoteNo.ToString(), "Note..." });
            lvi.Tag = vizcore3d.View.GetCameraData();
