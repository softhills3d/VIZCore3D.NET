...
namespace VIZCore3D.NET.VisualInspection
{
    public partial class FrmMain : Form
    {
        ...
        private void InitializeVIZCore3D()
        {
            ...
            vizcore3d.View.SelectionMode = VIZCore3D.NET.Data.Object3DSelectionOptions.FULL_BOUNDARIES;
        ...
        private void btnOpen_Click(object sender, EventArgs e)
        {
            ...
            lvModels.LargeImageList = imgList;
            lvModels.SmallImageList = imgList;
            lvModels.StateImageList = imgList;

            for (int i = 0; i < files.Length; i++)
            {
                string name = System.IO.Path.GetFileName(files[i]);
                string ext = System.IO.Path.GetExtension(files[i]).ToUpper();
                if (ext == ".VIZXML") continue;

                System.Drawing.Image img = VIZCore3D.NET.Manager.ModelManager.GetModelThumbnail(files[i]);
                if (img == null)
                    img = vizcore3d.Model.GetModelThumbnail(files[i], VIZCore3D.NET.Data.CameraDirection.ISO_PLUS, 90, 90);

                if (img == null) continue;

                imgList.Images.Add(name, img);

                VIZCore3D.NET.Data.BoundBox3D box = VIZCore3D.NET.Manager.ModelManager.GetModelBoundBox(files[i]);
        ...
        private void lvModels_DoubleClick(object sender, EventArgs e)
        {
            if (lvModels.SelectedItems.Count == 0) return;

            string path = (string)lvModels.SelectedItems[0].Tag;
        ...
        private void AnalyzeData()
        {
            ...
            List<VIZCore3D.NET.Data.Node> nodes = vizcore3d.Object3D.FromFilter(VIZCore3D.NET.Data.Object3dFilter.PART, false);

            float countX = (boundBox.LengthX / 1000.0f) + 1;
            float countY = (boundBox.LengthY / 1000.0f) + 1;
            float countZ = (boundBox.LengthZ / 1000.0f) + 1;

            List<int> vCountX = new List<int>();
            List<int> vCountY = new List<int>();
            List<int> vCountZ = new List<int>();
            ...
            {
                VIZCore3D.NET.Data.Vertex3D center = nodes[i].GetCenter();

                float fx = (Math.Max(boundBox.MinX, center.X) - Math.Min(boundBox.MinX, center.X)) / 1000.0f;
                vCountX[Convert.ToInt32(fx)]++;

                float fy = (Math.Max(boundBox.MinY, center.Y) - Math.Min(boundBox.MinY, center.Y)) / 1000.0f;
                vCountY[Convert.ToInt32(fy)]++;

                float fz = (Math.Max(boundBox.MinZ, center.Z) - Math.Min(boundBox.MinZ, center.Z)) / 1000.0f;
                vCountZ[Convert.ToInt32(fz)]++;
            }

            UpdateChart(chart1, vCountX);
            UpdateChart(chart2, vCountY);
            UpdateChart(chart3, vCountZ);
        }

        private void UpdateChart(System.Windows.Forms.DataVisualization.Charting.Chart chart, List<int> val)
        {
            chart.Series[0].Points.Clear();

            for (int i = 0; i < val.Count; i++)
            {
                chart.Series[0].Points.AddXY(i, val[i]);
            }
        ...
        private void btnUpdateThumbnail_Click(object sender, EventArgs e)
        {
            OpenFileDialog dlg = new OpenFileDialog();
            dlg.Filter = vizcore3d.Model.OpenFilter;
            dlg.Multiselect = true;
            ...
            foreach (string item in files)
            {
                ...
                vizcore3d.Model.SaveAsVIZ(item);
