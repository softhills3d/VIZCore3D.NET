...
namespace VIZCore3D.NET.AnalysisModel
{
    public partial class FrmMain : Form
    {
        ...
        public VIZCore3D.NET.Data.Node Node_Org { get; set; }
        public VIZCore3D.NET.Data.Node Node_Aly { get; set; }
        ...
        private void InitializeVIZCore3D()
        {
            ...
            // 선택 가능 개체 : 전체, 불투명한 개체, 투명한 개체
            vizcore3d.View.SelectionObject3DType = VIZCore3D.NET.Data.SelectionObject3DTypes.TRANSPARENCY_OBJECT3D;
        ...
        private void btnOpenDesign_Click(object sender, EventArgs e)
        {
            ...
            dlg.Filter = "Files (*.viz;*.stl)|*.viz;*.stl";
            if (dlg.ShowDialog() != DialogResult.OK) return;

            txtPathModel1.Text = dlg.FileName;
        }
        ...
        private void btnOpenAnalysis_Click(object sender, EventArgs e)
        {
            ...
            txtPathModel2.Text = dlg.FileName;
        }

        private void btnViewModel_Click(object sender, EventArgs e)
        {
            List<string> files = new List<string>();
            files.Add(txtPathModel1.Text);
            files.Add(txtPathModel2.Text);
        ...
        private void Highlight()
        {
            ...
            {
                if (node.NodeName == name_org)
                    Node_Org = node.GetChildObject3d(Data.Object3DChildOption.CHILD_ONLY)[0];
                else if (node.NodeName == name_aly)
                    Node_Aly = node.GetChildObject3d(Data.Object3DChildOption.CHILD_ONLY)[0];
            }

            vizcore3d.Object3D.Color.SetColorAndAlpha(new List<VIZCore3D.NET.Data.Node>() { Node_Org }, Color.Orange, 50);
        }

        private void btnPickSurface_Click(object sender, EventArgs e)
        {
        ...
        {
            if (e.Kind != VIZCore3D.NET.Data.OsnapKind.SURFACE) return;

            VIZCore3D.NET.Data.Vertex3D point = e.Point;
            VIZCore3D.NET.Data.Vector3D normal = e.Normal;
            VIZCore3D.NET.Data.Vector3D rnormal = (normal * -1);

            VIZCore3D.NET.Data.Vector3D arrow1 = point.ToVector3D().PointToVector(normal, 300.0f);
            VIZCore3D.NET.Data.Vector3D arrow2 = point.ToVector3D().PointToVector(rnormal, 300.0f);

            VIZCore3D.NET.Data.Vector3D textPt1 = point.ToVector3D().PointToVector(normal, 1000.0f);
            VIZCore3D.NET.Data.Vector3D textPt2 = point.ToVector3D().PointToVector(rnormal, 1000.0f);


            System.Diagnostics.Trace.WriteLine(string.Format("SURFACE : {0} / NORMAL : {1}", point.ToString(), normal.ToString()));

            {
                List<VIZCore3D.NET.Data.Vertex3DItemCollection> items = new List<VIZCore3D.NET.Data.Vertex3DItemCollection>();
                VIZCore3D.NET.Data.Vertex3DItemCollection item = new VIZCore3D.NET.Data.Vertex3DItemCollection();
                item.Add(arrow1.ToVertex3D());
                item.Add(point);
                items.Add(item);

                //vizcore3d.ShapeDrawing.AddArrow(items, 0, Color.Blue, Color.Red, 5, 15, true);
            }

            {
                List<VIZCore3D.NET.Data.Vertex3DItemCollection> items = new List<VIZCore3D.NET.Data.Vertex3DItemCollection>();
                VIZCore3D.NET.Data.Vertex3DItemCollection item = new VIZCore3D.NET.Data.Vertex3DItemCollection();
                item.Add(arrow2.ToVertex3D());
                item.Add(point);
                items.Add(item);

                //vizcore3d.ShapeDrawing.AddArrow(items, 0, Color.Red, Color.Blue, 5, 15, true);
            }

            {
                /*
                VIZCore3D.NET.Data.NearestObjectByAxisPoint np1 
                    = vizcore3d.GeometryUtility.GetNearestObject(point, normal.ToVertex3D());

                VIZCore3D.NET.Data.NearestObjectByAxisPoint np2
                   = vizcore3d.GeometryUtility.GetNearestObject(point, rnormal.ToVertex3D());

                System.Diagnostics.Trace.WriteLine(string.Format("INDEX : {0} / DIST. : {1}", np1.Index, np1.Distance));
                System.Diagnostics.Trace.WriteLine(string.Format("INDEX : {0} / DIST. : {1}", np2.Index, np2.Distance));

                if (np1.Index != -1 && np1.Distance > 0)
                {
                    vizcore3d.Review.Note.AddNoteSurface(np1.Distance.ToString(), textPt.ToVertex3D(), point);
                }

                if (np2.Index != -1 && np2.Distance > 0)
                {
                    vizcore3d.Review.Note.AddNoteSurface(np2.Distance.ToString(), textPt.ToVertex3D(), point);
                }
                */

                this.Cursor = Cursors.WaitCursor;

                List<VIZCore3D.NET.Data.NearestObjectByAxisPoint> np1s =
                vizcore3d.GeometryUtility.GetNearestObjects(point, normal.ToVertex3D(), 0, true, false);

                List<VIZCore3D.NET.Data.NearestObjectByAxisPoint> np2s =
                vizcore3d.GeometryUtility.GetNearestObjects(point, rnormal.ToVertex3D(), 0, true, false);

                List<VIZCore3D.NET.Data.NearestObjectByAxisPoint> items = new List<Data.NearestObjectByAxisPoint>();
                items.AddRange(np1s);
                items.AddRange(np2s);
                ...
                {
                    if (item.Index == Node_Org.Index) continue;

                    System.Diagnostics.Trace.WriteLine(string.Format("INDEX : {0} / DIST. : {1}", item.Index, item.Distance));

                    /*
                    VIZCore3D.NET.Data.MultiColorText text = new VIZCore3D.NET.Data.MultiColorText();
                    text.AddLine(string.Format("POINT : {0}", point.ToString()), Color.Black);
                    text.AddLine(string.Format("DISTANCE : {0}", item.Distance.ToString()), Color.Red);
                    */

                    string text = string.Format("{0} [mm]", item.Distance.ToString());

                    vizcore3d.Review.Note.AddNoteSurface(text, textPt1.ToVertex3D(), point);
        ...
        private void ckDepthTest_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                vizcore3d.Review.EnableDepthTest(item.ID, ckDepthTest.Checked);
            }
            vizcore3d.EndUpdate();
        }
    }
