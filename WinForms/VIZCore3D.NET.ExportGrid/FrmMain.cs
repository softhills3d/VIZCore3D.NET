...
namespace VIZCore3D.NET.ExportGrid
{
    public partial class FrmMain : Form
    {
        ...
        /// 이벤트 등록
        /// </summary>
        #region Function - VIZCore3D.NET : InitializeEvent
        ...
        private void Section_OnSectionEvent(object sender, Event.EventManager.SectionEventArgs e)
        {
            ...
            //단면 상자가 아닐 경우 리턴
            if(e.Section.SectionType != VIZCore3D.NET.Manager.SectionManager.SectionTypes.SECTION_BOX) return;

            // 이동 및 회전 이벤트
            //if(e.EventType == VIZCore3D.NET.Manager.SectionManager.EventTypes.TRANSFORM)
            //{
            //    VIZCore3D.NET.Data.BoundBox3D boundBox = e.Section.BoundBox;
            //}
            // 이동 완료 이벤트
            if (e.EventType == VIZCore3D.NET.Manager.SectionManager.EventTypes.RESIZED)
            {
                VIZCore3D.NET.Data.BoundBox3D boundBox = e.Section.BoundBox;
                txtMinX.Text = boundBox.MinX.ToString();
                txtMinY.Text = boundBox.MinY.ToString();
                txtMinZ.Text = boundBox.MinZ.ToString();
                txtMaxX.Text = boundBox.MaxX.ToString();
                txtMaxY.Text = boundBox.MaxY.ToString();
                txtMaxZ.Text = boundBox.MaxZ.ToString();
            } else if (e.EventType == VIZCore3D.NET.Manager.SectionManager.EventTypes.ADD)
            {
                VIZCore3D.NET.Data.BoundBox3D boundBox = e.Section.BoundBox;
                txtMinX.Text = boundBox.MinX.ToString();
                txtMinY.Text = boundBox.MinY.ToString();
        ...
        private void btnExportGrid_Click(object sender, EventArgs e)
        {
            ...
            VIZCore3D.NET.Data.BoundBox3D boundBox = new Data.BoundBox3D(
                txtMinX.Text.ToString(),
            txtMinY.Text.ToString(),
            txtMinZ.Text.ToString(),
            txtMaxX.Text.ToString(),
            txtMaxY.Text.ToString(),
            txtMaxZ.Text.ToString()
                );
        
            vizcore3d.Model.ExportGrid(path, boundBox, false);
        }

        private void btnExportGridReverse_Click(object sender, EventArgs e)
        {
            string path = txtExportReversePath.Text;
            VIZCore3D.NET.Data.BoundBox3D boundBox = new Data.BoundBox3D(
                txtMinX.Text.ToString(),
            txtMinY.Text.ToString(),
            txtMinZ.Text.ToString(),
            ...
            vizcore3d.Model.ExportGridReverse(path, boundBox, false);
        }
