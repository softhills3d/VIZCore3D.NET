...
namespace VIZCore3D.NET.SimplifyBlockByPart
{
    public partial class FrmMain : Form
    {
        ...
        private string Output_Simplify { get; set; }
        private Dictionary<int, int> BackupID { get; set; }
        ...
        private void btnSelect_Click(object sender, EventArgs e)
        {
            ...
            Output_Simplify = System.IO.Path.Combine(Dir, string.Format("{0}-SIMPLIFY", FileName));
            if (System.IO.Directory.Exists(Output_Simplify) == false)
                System.IO.Directory.CreateDirectory(Output_Simplify);
        }
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            List<int> exportNode = new List<int>();
            foreach (VIZCore3D.NET.ShdCore.ModelTreeNode node in nodes)
            {
                if (node.NodeType != ShdCore.ModelTreeNodeKind.PART) continue;

                exportNode.Add(Convert.ToInt32(node.EntityId));
            }
        ...
        private void btnGenerateTree_Click(object sender, EventArgs e)
        {
            ...
            foreach (VIZCore3D.NET.ShdCore.ModelTreeNode node in nodes)
            {
                if (node.NodeType == VIZCore3D.NET.ShdCore.ModelTreeNodeKind.BODY) continue;
        ...
        private void btnLinkModel_Click(object sender, EventArgs e)
        {
            ...
            {
                if (BackupID.ContainsKey(node.ID) == false) continue;

                //vizcore3d.Structure.LinkExternalFile(node.Index, string.Format("{0}\\{1}.viz", Output_Simplify, BackupID[node.ID]), false);

                vizcore3d.Structure.LinkExternalNode(
                    node.Index
                    , string.Format("{0}\\{1}.viz", Output_Simplify, BackupID[node.ID])
                    , string.Format("\\{0}", BackupID[node.ID].ToString())
                    , false
                    );
            }
