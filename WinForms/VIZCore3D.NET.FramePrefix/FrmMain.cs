...
namespace VIZCore3D.NET.FramePrefix
{
    public partial class FrmMain : Form
    {
        ...
        {
            string path = string.Format("{0}\\Models\\VIZCore3D.NET.FramePrefix\\H1195_V303.viz", vizcore3d.GetEntryAssemblyPath());
        ...
        private void buttonFrameOpen_Click(object sender, EventArgs e)
        {
            ...
            string path = string.Format("{0}\\Models\\VIZCore3D.NET.FramePrefix\\H1195.DMP", vizcore3d.GetEntryAssemblyPath());
        ...
        {
            bool isID = radioButtonID.Checked;
            bool isSetPrefix = checkBoxSetPrefix.Checked;
            ...
            if (isSetPrefix == true)
            {
                ...
                {
                    MessageBox.Show("Prefix를 입력해주세요.", "VIZCore3D.NET.FramePrefix", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
            if(checkBoxX.Checked == false && checkBoxY.Checked == false && checkBoxZ.Checked == false)
            {
                MessageBox.Show("반영할 축을 선택해주세요.", "VIZCore3D.NET.FramePrefix", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if(isID == true)                // ID
            {
                if(isSetPrefix == true)     // Prefix
                {
                    if(checkBoxX.Checked == true)
                    {
                        SetFrameTypePrefixX(false, prefix);
                    }
                    if (checkBoxY.Checked == true)
                    {
                        SetFrameTypePrefixY(false, prefix);
                    }
                    if (checkBoxZ.Checked == true)
                    {
                        SetFrameTypePrefixZ(false, prefix);
                    }
                }
                else
                {
                    if (checkBoxX.Checked == true)
                    {
                        SetFrameTypePrefixX(false, "");
                    }
                    if (checkBoxY.Checked == true)
                    {
                        SetFrameTypePrefixY(false, "");
                    }
                    if (checkBoxZ.Checked == true)
                    {
                        SetFrameTypePrefixZ(false, "");
                    }
                }
            }
            else                            // Position
            {
                if (isSetPrefix == true)    // Prefix
                {
                    if (checkBoxX.Checked == true)
                    {
                        SetFrameTypePrefixX(true, prefix);
                    }
                    if (checkBoxY.Checked == true)
                    {
                        SetFrameTypePrefixY(true, prefix);
                    }
                    if (checkBoxZ.Checked == true)
                    {
                        SetFrameTypePrefixZ(true, prefix);
                    }
                }
                else
                {
                    if (checkBoxX.Checked == true)
                    {
                        SetFrameTypePrefixX(true, "");
                    }
                    if (checkBoxY.Checked == true)
                    {
                        SetFrameTypePrefixY(true, "");
                    }
                    if (checkBoxZ.Checked == true)
                    {
                        SetFrameTypePrefixZ(true, "");
                    }
                }
            }
        }

        private void SetFrameTypePrefixX(bool type, string prefix)
        {
            vizcore3d.Frame.XAxisLabelAsPosition = type;
            if(type == true)    // Position
                vizcore3d.Frame.XAxisFramePositionPrefix = prefix;
            else                // ID
                vizcore3d.Frame.XAxisFrameIDPrefix = prefix;
        }

        private void SetFrameTypePrefixY(bool type, string prefix)
        {
            vizcore3d.Frame.YAxisLabelAsPosition = type;
            if (type == true)    // Position
                vizcore3d.Frame.YAxisFramePositionPrefix = prefix;
            else                // ID
                vizcore3d.Frame.YAxisFrameIDPrefix = prefix;
        }

        private void SetFrameTypePrefixZ(bool type, string prefix)
        {
            vizcore3d.Frame.ZAxisLabelAsPosition = type;
            if (type == true)    // Position
                vizcore3d.Frame.ZAxisFramePositionPrefix = prefix;
            else                // ID
                vizcore3d.Frame.ZAxisFrameIDPrefix = prefix;
        }
    }
}
