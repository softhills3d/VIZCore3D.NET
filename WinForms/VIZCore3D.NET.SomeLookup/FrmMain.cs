...
namespace VIZCore3D.NET.SomeLookup
{
    public partial class FrmMain : Form
    {
        ...
        /// <summary>
        /// VIZCore3D.NET (Detail View)
        /// </summary>
        private VIZCore3D.NET.VIZCore3DControl vizcore3dDetail;
        ...
        public FrmMain()
        {
            ...
            // Construction
            vizcore3dDetail = new VIZCore3D.NET.VIZCore3DControl();

            vizcore3dDetail.Dock = DockStyle.Fill;
            groupBox2.Controls.Add(vizcore3dDetail);

            // Event
            vizcore3dDetail.OnInitializedVIZCore3D += VIZCore3DDetail_OnInitializedVIZCore3D;
        }
        ...
        #region Event - OnInitializedVIZCore3D
        private void VIZCore3DDetail_OnInitializedVIZCore3D(object sender, EventArgs e)
        {
            //MessageBox.Show("OnInitializedVIZCore3D", "VIZCore3D.NET");
            ...
            if (licenseData == VIZCore3D.NET.Utility.LicenseHelper.LicenseData.SERVER)
            {
                licenseResult = vizcore3dDetail.License.LicenseServer(
                    licenseInfo.ContainsKey("LICENSE_IP") == true ? licenseInfo["LICENSE_IP"] : String.Empty
                    , licenseInfo.ContainsKey("LICENSE_PORT") == true ? Convert.ToInt32(licenseInfo["LICENSE_PORT"]) : 8901
                    );
            }
            ...
            {
                licenseResult = vizcore3dDetail.License.LicenseFile(
                    licenseInfo.ContainsKey("LICENSE_FILE") == true ? licenseInfo["LICENSE_FILE"] : String.Empty
                    );
            }
            ...
            // Init. VIZCore3D.NET
            vizcore3dDetail.View.PreSelect.Enable = false;
            vizcore3dDetail.Statusbar.Visible = false;
            vizcore3dDetail.View.MarineAxis.Visible = false;
            vizcore3dDetail.View.SilhouetteEdge = true;
        }
        #endregion
        ...
        private void btnShowModel_Click(object sender, EventArgs e)
        {
            ...
            {
                string path = string.Format("{0}\\Nodes.viz", Application.StartupPath);
                if (System.IO.File.Exists(path) == true) System.IO.File.Delete(path);

                vizcore3d.Model.ExportVIZ(path, VIZCore3D.NET.Data.ModelExportFilter.SELECTED_ONLY);

                // Closing a file because it doesn't open the same file
                vizcore3dDetail.Model.Close();    
                vizcore3dDetail.Model.Open(path);
            }
            // Generate VIZXML
            else if (rbVIZXML.Checked == true)
            {
                string model = vizcore3d.Model.Files[0];

                VIZCore3D.NET.Manager.VIZXMLManager xml = new VIZCore3D.NET.Manager.VIZXMLManager("MODEL");
                VIZCore3D.NET.Manager.VIZXMLNode root = new VIZCore3D.NET.Manager.VIZXMLNode("ROOT");

                foreach (VIZCore3D.NET.Data.Node node in nodes)
                {
                    VIZCore3D.NET.Manager.VIZXMLNode assy = new Manager.VIZXMLNode(
                        node.NodeName
                        , model
                        , node.ID
                        , node.Kind == VIZCore3D.NET.Data.NodeKind.ASSEMBLY 
                        ? VIZCore3D.NET.Manager.VIZXML_MODEL_KIND.Assembly 
                        : VIZCore3D.NET.Manager.VIZXML_MODEL_KIND.Part
                        );
                ...
                string path = string.Format("{0}\\Nodes.vizxml", Application.StartupPath);
                if (System.IO.File.Exists(path) == true) System.IO.File.Delete(path);

                xml.ExportVIZXML(path);
                ...
                // Show Root Node
                vizcore3dDetail.Object3D.Show(new List<int>() { 0 }, true);
            }
            // Create Node
            else if(rbLinkNode.Checked == true)
            {
                string model = vizcore3d.Model.Files[0];

                // New Empty Model
                int index = vizcore3dDetail.Model.NewEmptyModel("MODEL");

                string assemblyName = "ROOT";
                VIZCore3D.NET.Data.Node assemblyNode =
                    vizcore3dDetail.Structure.CreateNode(
                ...
                foreach (VIZCore3D.NET.Data.Node node in nodes)
                {
                    if (node.Kind == VIZCore3D.NET.Data.NodeKind.ASSEMBLY)
                    {
                        VIZCore3D.NET.Data.Node assyNode =
                            vizcore3dDetail.Structure.CreateNode(
                        ...
                        vizcore3dDetail.Structure.LinkExternalNode(assyNode.Index, model, vizcore3d.Object3D.GetNodePath(node.Index));
                    }
                    else if(node.Kind == VIZCore3D.NET.Data.NodeKind.PART)
                    {
                        ...
                        vizcore3dDetail.Structure.LinkExternalNode(partNode.Index, model, vizcore3d.Object3D.GetNodePath(node.Index));
                    }
                }
                

                vizcore3dDetail.Structure.RebuildData();

                // Show Root Node
                vizcore3dDetail.Object3D.Show(new List<int>() { 1 }, true);
            }
        }
    }
}
