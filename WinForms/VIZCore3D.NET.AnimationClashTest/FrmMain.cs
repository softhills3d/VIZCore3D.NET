...
namespace VIZCore3D.NET.AnimationClashTest
{
    public partial class FrmMain : Form
    {
        ...
        List<ClashTestPathItem> pathItems = new List<ClashTestPathItem>();
        ...
        private void ShowResultList()
        {
            ...
            {
                Thread thread = new Thread(() => StartClashTest(pathItems[progressIndex]));
                thread.Start();
            }
            else
            {
        ...
        private void btnPerformTest_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("시작 지점을 설정해주세요.", "VIZCore3D.NET.ClashTest", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        ...
        private void StartClashTest(ClashTestPathItem path)
        {
            vizcore3d.BeginUpdate();
            // 이동할 거리를 설정합니다.
            float X = path.X;
            ...
            vizcore3d.Object3D.Select(groupB, true);
            vizcore3d.Object3D.Transform.Move(X, Y, Z, true);
            vizcore3d.Object3D.Select(Data.Object3dSelectionModes.DESELECT_ALL);
            vizcore3d.EndUpdate();
        ...
        private void btnClear_Click(object sender, EventArgs e)
        {
            vizcore3d.Clash.Clear();
            clash = new Data.ClashTest();
            pathItems = new List<ClashTestPathItem>();
            progressIndex = 0;
            this.Invoke(new EventHandler(delegate
            {
                SetPathList();
            ...
            MessageBox.Show(string.Format("Clash Clear"));
        }
        ...
        private void btnAddPath_Click(object sender, EventArgs e)
        {
            ...
            VIZCore3D.NET.Data.Axis axis = (VIZCore3D.NET.Data.Axis)cbAxis.SelectedIndex;
            float distance = Convert.ToSingle(txtDistance.Text);


            ClashTestPathItem path = new ClashTestPathItem();
            ClashTestPathItem lastPath = pathItems[pathItems.Count - 1];
            path.X = lastPath.X;
            path.Y = lastPath.Y;
            path.Z = lastPath.Z;
            ...
            switch (axis)
            {
                case VIZCore3D.NET.Data.Axis.X:
                    path.X += distance;
                    break;
                case VIZCore3D.NET.Data.Axis.Y:
                    path.Y += distance;
                    break;
                case VIZCore3D.NET.Data.Axis.Z:
                    path.Z += distance;
                    break;
            ...
            this.Activate();
            //MessageBox.Show("경로가 추가되었습니다.", "VIZCore3D.NET.ClashTest", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
        ...
        {
            if(clash.Path != null && clash.Path.Count > 0)
            {
                MessageBox.Show("이미 시작 지점이 설정되었습니다.", "VIZCore3D.NET.ClashTest", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            ClashTestPathItem position = new ClashTestPathItem();
            position.X = Convert.ToSingle(txtStartPosX.Text);
            position.Y = Convert.ToSingle(txtStartPosY.Text);
            position.Z = Convert.ToSingle(txtStartPosZ.Text);
            pathItems.Add(position);
            this.Invoke(new EventHandler(delegate
            {
                SetPathList();
            ...
            this.Activate();
            MessageBox.Show("시작 지점이 설정되었습니다.", "VIZCore3D.NET.ClashTest", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        }
        ...
        private void SetPathList()
        {
            ...
            {
                ClashTestPathItem path = pathItems[i];
