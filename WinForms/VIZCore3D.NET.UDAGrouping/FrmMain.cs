...
namespace VIZCore3D.NET.UDAGrouping
{
    public partial class FrmMain : Form
    {
        ...
        private readonly string GROUP_KEY = "GROUP";
        ...
        {
            // Model
            vizcore3d.Model.OnModelOpenedEvent += Model_OnModelOpenedEvent; // Open
            vizcore3d.Model.OnModelClosedEvent += Model_OnModelClosedEvent; // Close
        ...
        #region Function - Init. Example
        private void InitExample()
        {
            vizcore3d.View.SetContextMenu(VIZCore3D.NET.Data.ViewContextMenuKind.NODE_SINGLE, contextMenuGroup);
            vizcore3d.View.SetContextMenu(VIZCore3D.NET.Data.ViewContextMenuKind.NODE_MULTI, contextMenuGroup);
        }
        #endregion
        ...
        private void UpdateGroupList()
        {
            ...
            List<string> vals = vizcore3d.Object3D.UDA.GetValues(GROUP_KEY);

            foreach (string item in vals)
            {
                List<int> items = vizcore3d.Object3D.UDA.GetNodesIndex(GROUP_KEY, item);

                ListViewItem lvi = new ListViewItem(new string[] { item, string.Format("{0:#,0}", items.Count) });
        ...
        private void btnAddGroup_Click(object sender, EventArgs e)
        {
            ...
            if(nodes.Count == 0)
            {
                MessageBox.Show("Selected Item is null.", "VIZCore3D.NET", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            List<string> vals = vizcore3d.Object3D.UDA.GetValues(GROUP_KEY);

            FrmNewGroup dlg = new FrmNewGroup();
            dlg.GroupItems = vals;
            if (dlg.ShowDialog() != DialogResult.OK) return;

            string title = dlg.GroupTitle;

            AddGroup(title, nodes);
        }
        ...
        private void btnRemoveGroup_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Selected List Item is null.", "VIZCore3D.NET", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            ListViewItem lvi = lvGroup.SelectedItems[0];
            string title = lvi.SubItems[0].Text;

            List<int> index = vizcore3d.Object3D.UDA.GetNodesIndex(GROUP_KEY, title);
            foreach (int item in index)
            {
                vizcore3d.Object3D.UDA.Delete(item, GROUP_KEY, false);
            }

            UpdateGroupList();
        }

        private void AddGroup(string title, List<VIZCore3D.NET.Data.Node> nodes)
        {
            foreach (VIZCore3D.NET.Data.Node node in nodes)
            {
                Dictionary<string, string> uda = vizcore3d.Object3D.UDA.FromIndex(node.Index, false);

                vizcore3d.Object3D.UDA.Add(node.Index, GROUP_KEY, title, false);
            }
        ...
        private void HighlightItems()
        {
            ...
            bool xray = ckXray.Checked;

            vizcore3d.BeginUpdate();

            vizcore3d.View.XRay.Enable = xray;
            ...
            if (xray == true)
            {
                vizcore3d.View.XRay.Select(index, true);
            }
            else
            {
        ...
        {
            bool enableGroupSelection = ckEnableGroup.Checked;

            if (enableGroupSelection == false) return;

            Dictionary<string, string> uda = vizcore3d.Object3D.UDA.FromIndex(e.Node.Index);
            if (uda.ContainsKey(GROUP_KEY) == false) return;

            string groupName = uda[GROUP_KEY];
        ...
        private void Object3D_OnObject3DSelected(object sender, Event.EventManager.Object3DSelectedEventArgs e)
        {
            ...
            Dictionary<string, string> uda = vizcore3d.Object3D.UDA.FromIndex(e.Node[0].Index);
            if (uda.ContainsKey(GROUP_KEY) == false) return;
        ...
        // ================================================
        private void menuNewGroup_Click(object sender, EventArgs e)
        {
            List<string> vals = vizcore3d.Object3D.UDA.GetValues(GROUP_KEY);
        ...
        private void View_OnViewContextMenuOpeningEvent(object sender, Event.EventManager.ViewContextMenuEventArgs e)
        {
            if (e.Kind == VIZCore3D.NET.Data.ViewContextMenuKind.NODE_SINGLE || e.Kind == VIZCore3D.NET.Data.ViewContextMenuKind.NODE_MULTI)
            {
                menuUpdateGroup.DropDownItems.Clear();

                List<string> vals = vizcore3d.Object3D.UDA.GetValues(GROUP_KEY);
                foreach (string item in vals)
                {
                    System.Windows.Forms.ToolStripMenuItem group = new System.Windows.Forms.ToolStripMenuItem();
                    group.Text = item;
                    group.Click += GroupItem_Click;

                    menuUpdateGroup.DropDownItems.Add(group);
                }
            }
        }
        ...
        {
            System.Windows.Forms.ToolStripMenuItem group = (System.Windows.Forms.ToolStripMenuItem)sender;

            List<VIZCore3D.NET.Data.Node> nodes = vizcore3d.Object3D.FromFilter(VIZCore3D.NET.Data.Object3dFilter.SELECTED_PART);

            foreach (VIZCore3D.NET.Data.Node node in nodes)
            {
                vizcore3d.Object3D.UDA.Delete(node.Index, GROUP_KEY, false);
            }

            AddGroup(group.Text, nodes);
        }
    }
}
