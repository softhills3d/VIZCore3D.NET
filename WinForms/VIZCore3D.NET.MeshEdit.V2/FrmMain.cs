...
namespace VIZCore3D.NET.MeshEdit.V2
{
    public partial class FrmMain : Form
    {
        ...
        // Event
        // ================================================
        private void btnPasteStart_Click(object sender, EventArgs e)
        {
            txtStart.Text = Clipboard.GetText();
        }

        private void btnPasteEnd_Click(object sender, EventArgs e)
        {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (String.IsNullOrEmpty(txtStart.Text) == true) return;
            if (String.IsNullOrEmpty(txtEnd.Text) == true) return;
        ...
        private void btnAddBulk_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                if (item.Kind == VIZCore3D.NET.Data.NodeKind.PART) continue;
        ...
        // Function
        // ================================================
        private bool ExistGroupNode(string name)
        {
            List<VIZCore3D.NET.Data.Node> nodes = vizcore3d.Object3D.Find.QuickSearch(name, true);
            return nodes.Count == 0 ? false : true;
        }

        private VIZCore3D.NET.Data.Node AddGroupNode(string name)
        {
            VIZCore3D.NET.Data.Node group =
                vizcore3d.Structure.CreateNode(
                    0                                       /* Parent Index : 0 is Root */
        ...
        private VIZCore3D.NET.Data.Node GetGroupNode(string name)
        {
            List<VIZCore3D.NET.Data.Node> nodes = vizcore3d.Object3D.Find.QuickSearch(name, true);
        ...
        {
            List<VIZCore3D.NET.Data.Node> nodes = vizcore3d.Object3D.Find.QuickSearch(name, false);
            return nodes.Count;
        }

        private int CreateNode(string name, int parentIndex)
        {
            VIZCore3D.NET.Data.Node part = vizcore3d.Structure.CreateNode(parentIndex, VIZCore3D.NET.Data.NodeKind.PART, name);
            int bodyId = vizcore3d.Structure.CreateBody(part.Index, name);

            return vizcore3d.Object3D.GetBodyIndex(bodyId);
        }
