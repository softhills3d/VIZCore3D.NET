    ...
    public partial class FrmMain : Form
    {
        ...
        private void Object3D_OnObject3DSelected(object sender, Event.EventManager.Object3DSelectedEventArgs e)
        {
            if (e.Node.Count == 0)
                ...
                ShowCircleData(e.Node[0]);
        }

        private void ShowCircleData(VIZCore3D.NET.Data.Node node)
        {
            ClearCircleData();

            List<VIZCore3D.NET.Data.CircleData> items = vizcore3d.GeometryUtility.GetCircleData(node.Index);
            if (items.Count == 0) return;

            if (ckHighlightCenter.Checked == true)
                HighlightCenter(items);
