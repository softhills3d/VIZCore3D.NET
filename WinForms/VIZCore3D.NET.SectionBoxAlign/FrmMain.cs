...
namespace VIZCore3D.NET.SectionBoxAlign
{
    public partial class FrmMain : Form
    {
        ...
        private void btnAddSectionBox_Click(object sender, EventArgs e)
        {
            ...
            vizcore3d.Section.AddBox(false);
        }

        private void btnClearSectionBox_Click(object sender, EventArgs e)
        {
            if (vizcore3d.Model.IsOpen() == false) return;
        ...
        {
            if (vizcore3d.Section.Sections.Count == 0) return;
            if (e.Kind != VIZCore3D.NET.Data.OsnapKind.LINE) return;

            VIZCore3D.NET.Data.Vertex3D start = e.Start;
            VIZCore3D.NET.Data.Vertex3D end = e.End;

            VIZCore3D.NET.Data.Axis axis = Data.Axis.X;
            VIZCore3D.NET.Data.BoundBox3D boundBox = vizcore3d.Section.Sections[0].BoundBox;

            if (start.XToInt32() != end.XToInt32()) axis = VIZCore3D.NET.Data.Axis.X;
            else if (start.YToInt32() != end.YToInt32()) axis = VIZCore3D.NET.Data.Axis.Y;
            else axis = VIZCore3D.NET.Data.Axis.Z;

            switch (axis)
            {
                case Data.Axis.X:
                    {
                        float length1 = boundBox.MaxY - start.Y;
                        float length2 = start.Y - boundBox.MinY;

                        if (length1 < length2)
                        {
                            vizcore3d.Section.SetBoxSize(vizcore3d.Section.Sections[0].ID, boundBox.GetVertexMin(), new Data.Vertex3D(boundBox.MaxX, start.Y, boundBox.MaxZ));
                        }
                        else
                        {
                            vizcore3d.Section.SetBoxSize(vizcore3d.Section.Sections[0].ID, new Data.Vertex3D(boundBox.MinX, start.Y, boundBox.MinZ), boundBox.GetVertexMax());
                        }
                    }
                    break;
                case Data.Axis.Y:
                    {
                        float length1 = boundBox.MaxX - start.X;
                        float length2 = start.X - boundBox.MinX;

                        if (length1 < length2)
                        {
                            vizcore3d.Section.SetBoxSize(vizcore3d.Section.Sections[0].ID, boundBox.GetVertexMin(), new Data.Vertex3D(start.X, boundBox.MaxY, boundBox.MaxZ));
                        }
                        else
                        {
                            vizcore3d.Section.SetBoxSize(vizcore3d.Section.Sections[0].ID, new Data.Vertex3D(start.X, boundBox.MinY, boundBox.MinZ), boundBox.GetVertexMax());
                        }
                    }
                    break;
                case Data.Axis.Z:
