...
namespace VIZCore3D.NET.WebBrowserInterworking
{
    public partial class FrmMain : Form
    {
        ...
        private void Object3D_OnObject3DSelected(object sender, Event.EventManager.Object3DSelectedEventArgs e)
        {
            if (e.Node.Count == 0)
            {
                webView.Document.InvokeScript("clearEventTextBox");
            }
            else
            {
                webView.Document.InvokeScript("onObject3DSelected", new object[] { e.Node[0].Index, e.Node[0].NodeName });
            }
        }
        ...
        // Event - JavaScript
        // ================================================
        private void Javascript_OnOpenModel(object sender, JavaScriptOpenModelEventArgs e)
        {
            vizcore3d.Model.Close();

            if (String.IsNullOrEmpty(e.File) == true) return;

            string path = string.Format("{0}\\{1}", Application.StartupPath, e.File);
        ...
        private void Javascript_OnCloseModel(object sender, EventArgs e)
        {
            vizcore3d.Model.Close();
        }
    }
