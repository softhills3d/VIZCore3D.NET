...
namespace VIZCore3D.NET.MetadataDemo
{
    public partial class FrmMain : Form
    {
        ...
        private void btnRefresh_Click(object sender, EventArgs e)
        {
            ...
            if (count <= 0)
            {
                MessageBox.Show("Metadata is null.", "VIZCore3D.NET", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            AddMetadataDialog dlg = new AddMetadataDialog();
            if (dlg.ShowDialog() != DialogResult.OK) return;

            string name = dlg.MetadataName;
            string path = dlg.MetadataFilePath;

            int maxId = vizcore3d.Metadata.GetMaxId();

            bool result = vizcore3d.Metadata.Add(maxId + 1, name, System.IO.File.ReadAllBytes(path));
            ...
            {
                MessageBox.Show("Fail - Add Item", "VIZCore3D.NET", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            if (lvMetadata.SelectedItems.Count == 0) return;

            ListViewItem lvi = lvMetadata.SelectedItems[0];
            string id = lvi.SubItems[0].Text;
            string name = lvi.SubItems[1].Text;

            vizcore3d.Metadata.Delete(Convert.ToInt32(id));
        ...
        private Image ByteArrayToImage(byte[] buffer)
        {
            ...
            Image image = (Image)ic.ConvertFrom(buffer);
            return image;
        }
