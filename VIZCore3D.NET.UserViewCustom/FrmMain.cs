...
namespace VIZCore3D.NET.UserViewCustom
{
    public partial class FrmMain : Form
    {
        ...
        private string CVR_Path { get; set; }
        ...
        private string GetCVRPath(bool note, bool measure)
        {
            if (note == false && measure == false) return String.Empty;
            ...
            if (note == true)
            {
                ...
                foreach (VIZCore3D.NET.Data.MeasureItem item in measures)
                {
                    items.Add(item.ID);
                }
            }
            ...
            string export = string.Format("{0}\\REVIEW_{1}.cvr", CVR_Path, DateTime.Now.ToString("yyyyMMddHHmmss"));

            vizcore3d.Review.Export(export, items);
        ...
        private void lvItems_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (System.IO.File.Exists(cvr) == false) return;
