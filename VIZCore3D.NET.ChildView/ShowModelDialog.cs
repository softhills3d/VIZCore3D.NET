...
namespace VIZCore3D.NET.ChildView
{
    public partial class ShowModelDialog : Form
    {
        ...
        public ShowModelDialog(string path)
        {
            InitializeComponent();
        ...
        {
            if (String.IsNullOrEmpty(ModelPath) == true) return;
            if (System.IO.File.Exists(ModelPath) == false) return;

            vizcore3d.Model.Close();

            vizcore3d.Model.OpenStream("MODEL", System.IO.File.ReadAllBytes(ModelPath));
        }
